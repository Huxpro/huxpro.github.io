---
layout:     post
title:      "刚接触ubuntu时遇到的一些问题「一」"
subtitle:   "遇到的一些问题"
date:       2017-02-04 14:00:00
author:     "Alex"
...
> 把我刚开始接触ubuntu遇到的一些问题写出来，方便自己查阅，也方便遇到同样问题的人查阅。


**1.ubuntu首次给root用户设置密码**

新装好的ubuntu在。刚安装好root用户是没有密码的，没有密码我们就没法用root用户登录 。给root用户设置密码输入命令sudo passwd,然后系统会让你输入密码，回车后按提示输入两次root的密码（注意，输入时是不会提示*号的，直接输入即可）

，设置完成之后就可以用命令“su root"切换root用户登录了。

好了，我们开始吧，这场对决将会非常血腥。

![](http://img.blog.csdn.net/20150702165419174)

图片来源：[@_江南一点雨](http://blog.csdn.net/u012702547/article/details/46728593)

**2.Linux下tar.xz结尾的文件的解压方法**

因为要在Ubuntu上编译Python3.6.0，下载到了一份tar.xz结尾的压缩文件
...
也可以直接使用 tar xvJf  ***.tar.xz来解压

资料来源：[silvervi](http://blog.csdn.net/silvervi/article/details/6325698)

**3.Ubuntu 中源码安装编译Python 3.6.0的方法教程**

其他的软件类似

      #wget https://www.python.org/ftp/python/3.6.0/Python-3.6.0.tar.xz 
      #chmod +x Python-3.6.0.tar.xz 
...
**4.ubuntu上怎么设置默认python命令是执行python3而不是python2**

我装好的ubuntu上Python默认版本是2.7.12。但是我又编译安装了Python3.6.0。

但是默认的python命令是python2.7.12，我要想执行python3.6.0就必须输入python3。
...
图片来源：[@zodiacg](https://segmentfault.com/q/1010000003713912)
