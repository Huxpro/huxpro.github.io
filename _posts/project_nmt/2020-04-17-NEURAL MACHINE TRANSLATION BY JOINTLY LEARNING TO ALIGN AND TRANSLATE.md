...
3. RNN Encoder–Decoder的运作方式：
   - encoder把输入句子 $x=\left(x_{1}, \cdots, x_{T_{x}}\right)$ 转换为 $c=q\left(\left\{h_{1}, \cdots, h_{T_{x}}\right\}\right)$ 
   - decoder根据 **context vector** $c$ 和 **all the previously predicted words** $\left\{y_{1}, \cdots, y_{t-1}\right\}$ 来预测下一个词，$p(y=\prod_{t=1}^{T} p\left(y_{t}  \mid \left\{y_{1}, \cdots, y_{t-1}\right\}, c\right)$. 
